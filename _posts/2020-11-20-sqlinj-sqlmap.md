...
- 数学逻辑符号替换
- 大于号>替换 between 0 and 10
- 等于号=替换 between 10 and 10

> binary.py

- mysql中的BINARY关键字 
- mysql特性不区分大小写
- 查询中where语句中,字段前添加binary
- 强制字段进行二进制比较
...
> commalesslimit.py commalessmid

- mysql特性, 处理逗号,不能使用的情况
...
- oracle特性 11g 19c测试可行,但找不到文档
- `where 1=1 UNION ALL SELECT` -> `where 1=1DUNION ALL SELECT`
