...
由于Java对远程访问RMI Registry做了限制，只有源地址是localhost的时候，才能调用rebind、bind、unbind等方法。

所欲我们可以调用list方法查看Registry上存在对象，看看有没有危险的对象可以利用

### 可以控制lookup方法的参数

#### codebase

codebase是一个远程的地址比如http，ftp，告诉JVM去哪里找到这个类，类似CLASSPATH，只不过一个本地一个远程
...
在反序列化一个对象是，会去寻找对象的类，首先到CLASSPATH路径下没有找到就会用到codebase远程加载这个类，如果可以控制codebase就可以进行攻击

在RMI中，我们是可以将codebase随着序列化数据一起传输的，服务器在接收到这个数据后就会去CLASSPATH和指定的codebase寻找类，由于codebase被控制导致任意命令执行漏洞。


实际利用起来比较难，JAVA做了一些限制：

- java.rmi.server.useCodebaseOnly配置必须为false(允许加载远程对象)，如果该值为true则禁止引用远程对象
