...
![picture 0](https://alivk353.github.io/image/1ad1521721e9f801ff25a26d23474aecca0a27fb839d398c0d237d319f7e8eea.png)  

以UnicastRdf封装RMI注册中心的信息 接着跟进Util.createProxy()

![picture 3](https://alivk353.github.io/image/a286a1993e258a088c16662a53ecc3161ec007190fc683c3779a8bd3c6b333ff.png)  
...
一般配合ysoserial.exploit.JRMPListener:

反序列化了 UnicastRef，主动连接到JRMPListener的端口

JRMPListener 会利用 JRMP 协议返回一个精心构造的恶意gadget链的对象

### 8u232_b09的修复

![picture 0](https://alivk353.github.io/image/ff433032eac945cae0bdf4fc023a198bce42352ff7b3fed52ee2935c6c044123.png)  
...
UnicastRemoteObject 在反序列化时会自动执行导出Export操作

这个过程会启动一个监听端口或调用特定的 RMI 运行时方法

```java
//UnicastRemoteObject的反序列化过程
private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException {
    in.defaultReadObject(); // 1. 恢复成员变量 (包括 port 等)
    reexport();             // 2. 关键点：重新导出对象
}
