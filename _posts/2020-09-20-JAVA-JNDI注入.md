...
RMI和JDAP都支持解析`JNDI Reference`对象 可以控制的JNDI服务端返回一个恶意的`Reference`对象 进而远程加载`Reference`远程加载Factory类

当客户端通过 JNDI 查找时 若返回的是 Reference JNDI 会自动从指定的` codebase `地址下载该类的字节码 

并实例化它调用无参构造器 触发GET `/test.class` 导致恶意代码执行
